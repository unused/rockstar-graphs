...
const extendEdges = (vis, nodes, extEdges) => {
  ...
  edges.forEach(function (item, index, array) {
    let scaleX = d3.scaleLinear()
        .domain([d3.min(extEdges[index], function (d) { return d.event; }), d3.max(extEdges[index], function (d) { return d.event; })])
        .range([item.source.x, item.target.x]);

    extEdges[index].forEach(function (event, eventIndex, eventArray) {
        let x = scaleX(event.event);
        let y = item.source.y + ((item.target.y - item.source.y) / (item.target.x - item.source.x)) * (x - item.source.x);

        console.debug(x);
        console.debug(y);
        console.debug(event);

        let edge = vis.append("g").selectAll("circle.eventedges")
            .data([event])
            .enter()
            .append("circle")
